...
function ModelCard({
  ...
  const handleCopy = () => {
    ...
    toast.success(t("warRoom.copied"));
  };
      ...
      {rank === 1 && result?.available && !isLoading && (
        <div className="absolute top-3 left-3 z-10">
          <span className="flex items-center gap-1 bg-amber-500/20 text-amber-400 text-[10px] font-bold px-2 py-0.5 rounded-full border border-amber-500/30">
            <Trophy className="w-3 h-3" /> {t("warRoom.fastest")}
          </span>
        </div>
      )}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-3 py-6 text-center">
            ...
            <div>
              {result?.error?.includes("429") || result?.error?.includes("quota") ? (
                <>
                  <p className="text-sm font-medium text-muted-foreground mb-1">{t("warRoom.quotaExceeded")}</p>
                  <p className="text-xs text-muted-foreground/60">{t("warRoom.quotaExceededDesc")}</p>
                </>
              ) : result?.error?.includes("API key not configured") ? (
                <>
                  <p className="text-sm font-medium text-muted-foreground mb-1">{t("warRoom.modelDisabled")}</p>
                  <p className="text-xs text-muted-foreground/60">
                    {t("warRoom.modelDisabledPrefix")} <code className="bg-muted px-1 rounded text-[10px]">{name.replace("-", "_").toUpperCase()}_API_KEY</code> {t("warRoom.modelDisabledSuffix")}
                  </p>
                </>
              ) : (
                <>
                  <p className="text-sm font-medium text-muted-foreground mb-1">{t("warRoom.connError")}</p>
          ...
          <div>
            ...
            {isLong && (
              ...
              >
                {expanded ? <><ChevronUp className="w-3 h-3" /> {t("warRoom.less")}</> : <><ChevronDown className="w-3 h-3" /> {t("warRoom.more")}</>}
              </button>
            )}
          </div>
        )}
...
export default function WarRoom() {
  ...
  const warRoomMutation = trpc.agent.warRoom.useMutation({
    onSuccess: (data: ModelResult[]) => {
      ...
      toast.success(t('warRoom.battleComplete', { n: successful }));
    },
    onError: (err: { data?: { code?: string }; message: string }) => {
      setHasRun(false);
      setResults([]);
      if (err.data?.code === "TOO_MANY_REQUESTS") {
        toast.error(t('warRoom.dailyLimitReached'));
      } else {
        toast.error(t('warRoom.errorMsg', { msg: err.message }));
      }
    },
  });
  ...
  const handleJudge = async () => {
    ...
    if (available.length < 2) { toast.error(t('warRoom.needTwoAnswers')); return; }
    setIsJudging(true);
    try {
      ...
      const r = await fetch("/api/prompt-factory/test", {
        ...
        body: JSON.stringify({
          prompt: t('warRoom.judgePrompt', { n: available.length }),
          testInput: t('warRoom.judgeTestInput', { prompt, summaries }),
        }),
      });
      const d = await r.json();
      setJudgeResult(d.result || t('warRoom.judgeFailedResult'));
      toast.success(t('warRoom.judgeSuccess'));
    } catch { toast.error(t('warRoom.judgeFailed')); }
    setIsJudging(false);
  };
    ...
    return (
      <div className="h-screen flex items-center justify-center bg-background p-4">
        <div className="bg-card border border-border rounded-2xl p-8 max-w-md w-full text-center space-y-4">
          ...
          <p className="text-muted-foreground">{t('warRoom.loginPrompt')}</p>
  ...
  // ─── Main UI ──────────────────────────────────────────────────────
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col" dir={i18n.dir()}>
      {/* Top Bar */}
      <header className="h-12 bg-card border-b border-border flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <Link href="/agent" className="flex items-center gap-1.5 text-muted-foreground hover:text-primary transition-colors text-sm">
            <Bot className="w-3.5 h-3.5" /> {t('warRoom.agentTab')}
          </Link>
          <div className="w-px h-5 bg-border" />
          <Swords className="w-5 h-5 text-primary" />
          <span className="font-bold text-sm">AI War Room</span>
          <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{t('warRoom.modelBattle')}</span>
          <LanguageSwitcher />
        </div>
      ...
      <div className="flex-1 flex flex-col max-w-7xl w-full mx-auto px-4 py-6 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-4 space-y-3">
          <textarea
            ...
            placeholder={t('warRoom.promptPlaceholder')}
          ...
          <div className="flex items-center justify-between">
            <span className="text-xs text-muted-foreground">
              {t('warRoom.charCount', { n: prompt.length })} • {warRoomMutation.isPending ? t('warRoom.inBattle') : t('warRoom.ctrlEnterSend')}
            </span>
            <Button
            ...
            >
              {warRoomMutation.isPending ? (
                <><Loader2 className="w-4 h-4 animate-spin" /> {t('warRoom.inBattle')}</>
              ) : (
                <><Swords className="w-4 h-4" /> {t('warRoom.startBattle')}</>
              )}
            </Button>
          </div>
        </div>
          ...
          {results.length > 0 && !warRoomMutation.isPending && (
            ...
            >
              ...
              <div className="flex items-center gap-1 text-muted-foreground text-xs mr-auto">
                ...
                {t('warRoom.modelsAnsweredCount', { answered: results.filter(r => r.available && !r.error).length, total: results.length })}
              </div>
              {/* AI Judge Button */}
              <Button size="sm" variant="outline" onClick={handleJudge} disabled={isJudging || results.filter(r=>r.available && r.content).length < 2}
                ...
                {isJudging ? <><Loader2 className="w-3 h-3 animate-spin" /> {t('warRoom.judging')}</> : <><Trophy className="w-3 h-3" /> {t('warRoom.aiJudge')}</>}
              </Button>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <div className="text-center text-xs text-muted-foreground/50 pb-4">
          ...
          {t('warRoom.disabledModelsNote')} <code className="bg-muted px-1 rounded">OPENAI_API_KEY</code>،{" "}
          <code className="bg-muted px-1 rounded">GEMINI_API_KEY</code>،{" "}
          <code className="bg-muted px-1 rounded">DEEPSEEK_API_KEY</code> {t('warRoom.disabledModelsNoteEnd')}
        </div>
      </div>
    </div>
  );
