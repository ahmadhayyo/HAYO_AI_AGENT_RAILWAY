...
// All providers return the SAME TwelveData shape ({ status:"ok", values:[...] }
// NEWEST-first) so every downstream caller keeps working unchanged. This makes
// the trading engine independent of TwelveData credits: OANDA (broker-grade, if
// OANDA_API_TOKEN is set) is tried first, then keyless Yahoo Finance, then the
// original TwelveData key-rotation logic.
...
async function fetchFromOanda(symbol: string, interval: string, outputsize: number): Promise<any | null> {
...
async function fetchFromYahoo(symbol: string, interval: string, outputsize: number): Promise<any | null> {
...
async function fetchTwelveData(url: string): Promise<any> {
  ...
  try {
    const u = new URL(url);
    symbol = decodeURIComponent(u.searchParams.get("symbol") || "");
    interval = u.searchParams.get("interval") || "5min";
...
ملاحظة: الاتجاه الرئيسي ${trendFilter?.allowsBuy ? "📈 صاعد" : "📉 هابط"} — ${trendFilter?.allowsBuy ? "يفضّل الشراء" : "يفضّل البيع"}.`.trim();
        ...
        const systemPrompt = `أنت محلل أسواق مالية خبير ومتخصص في الفوركس والذهب والعملات الرقمية. ستحصل على تحليل شامل يتضمن مؤشرات تقنية متقدمة + إشارات من 15 استراتيجية تداول (منها بنية السوق وفيبوناتشي والدايفرجنس ونماذج الشموع) + نتائج فلاتر تأكيد + انحياز الإطار الأعلى (MTF) + فلتر أخبار حي + حُكم فني آلي.
